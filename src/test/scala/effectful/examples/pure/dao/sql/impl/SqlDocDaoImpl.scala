...
//    meta_dataFieldNames
  ...
  def prepFindById(implicit context: SqlDriver.Context): E[ID => E[Option[(ID,A,RecordMetadata)]]]  =
      sql.prepare(
        ...
        { (id:ID) =>
            sql.iteratePreparedQuery(ps)(Vector(recordFormat.toSqlVal(id)))
              .map(parseFullRecord)
              .headOption()
        }
      }
  ...
  override def findByNativeQuery(whereClause: SqlString): E[Vector[(ID, A, RecordMetadata)]] =
    sql.autoCommit { implicit autoCommit =>
...
//  def prepFindAll(implicit context: Context) : E[() => E[Vector[(ID,A,RecordMetadata)]]] =
...
//    }
  ...
  override def findAll(start: Int, batchSize: Int): E[Vector[(ID, A, RecordMetadata)]] =
    sql.autoCommit { implicit autoCommit =>
      prepFindAll.flatMap(_())
    }
  ...
  def prepExists(implicit context: Context) : E[ID => E[Boolean]] =
    sql.prepare(
      ...
      { (id:ID) =>
        for {
          cursor <- sql.executePreparedQuery(ps)(Vector(recordFormat.toSqlVal(id)))
    ...
    sql.prepare(
      ...
      { (id:ID) =>
        sql.executePreparedUpdate(ps)(
          Vector(SqlVal.TIMESTAMP(Instant.now()),recordFormat.toSqlVal(id))
        ).map(_ == 1)
      }
    }
  ...
  override def batchRemove(ids: Traversable[ID]): E[Int] = {
    sql.inTransaction { implicit transaction =>
      ...
      for {
        ...
        // todo:
        updateCounts <- implicitly[cats.Traverse[List]].sequence(ids.map(_remove).toList)
  ...
  def prepInsert(implicit context: Context) : E[Vector[(ID,A)] => E[Int]] =
    if(metadataTableSameAsRecord) {
      sql.prepare(
  ...
  override def insert(id: ID, a: A): E[Boolean] =
    sql.inTransaction { implicit transaction =>
      prepInsert.flatMap(_(Vector((id,a)))).map(_ == 1)
    }

  override def batchInsert(records: Traversable[(ID, A)]): E[Int] =
    sql.inTransaction { implicit transaction =>
      prepInsert.flatMap(_(records.toVector))
    }
  ...
  override def batchUpdate(records: Traversable[(ID, A)]): E[Int] =
    ...
    sql.inTransaction { implicit transaction =>
      ...
      for {
        _update <- prepUpdate
        // todo:
        results <- implicitly[cats.Traverse[List]].sequence(records.map { case (id,a) => _update(id,a) }.toList)
      ...
      result <- sql.inTransaction { implicit transaction =>
        for {
          ...
          result <- {
            import cats.std.list._
            // todo:
            val eUpdates = implicitly[cats.Traverse[List]].sequence(toUpdate.map { case (id,a) => _update(id,a) }.toList).map(_.count(_ == true))
            val eInserts = _insert(toInsert.toVector)
            for {
